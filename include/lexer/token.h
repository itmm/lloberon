...
public:
	...
	[[nodiscard]] llvm::SMLoc location() const {
		return llvm::SMLoc::getFromPointer(ptr_);
	}

	[[nodiscard]] llvm::StringRef identifier() const {
		assert(
			is(token::identifier) && "Cannot get identifier of non-identifier"
		);
		return { ptr_, length_ };
	}
	...
	[[nodiscard]] bool is_one_of(token::Kind first, Args... rest) const {
