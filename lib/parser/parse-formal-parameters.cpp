...
bool Parser::parse_formal_parameters(lloberon::sema::Formal_Parameters& formal_parameters) {
    if (consume(token::left_parenthesis)) { return true; }
    if (!token_.is(token::right_parenthesis)) {
        lloberon::sema::Formal_Parameter_Section formal_parameter_section { formal_parameters.scope() };
    ...
    if (token_.is(token::colon)) {
        advance();
        lloberon::sema::Qual_Ident qual_ident { formal_parameters.scope() };
