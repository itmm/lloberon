...
bool Parser::parse_statement(lloberon::sema::Statement& statement) {
    if (token_.is(token::identifier)) {
        ...
        lloberon::sema::If_Statement if_statement { statement.scope() };
        ...
        lloberon::sema::While_Statement while_statement { statement.scope() };
        if (parse_while_statement(while_statement)) { return true; }
    } else if (token_.is(token::keyword_REPEAT)) {
        lloberon::sema::Repeat_Statement repeat_statement { statement.scope() };
        if (parse_repeat_statement(repeat_statement)) { return true; }
    } else if (token_.is(token::keyword_FOR)) {
        lloberon::sema::For_Statement for_statement { statement.scope() };
